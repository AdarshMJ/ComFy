...
# def community_rewiring(data, seed, comm_delete, comm_add):
#     start_time = time.time()
    
#     random.seed(seed)
#     np.random.seed(seed)
#     print("Rewiring using community detection...")
...
#     inter_community_edges = [edge for edge in graph.edges() 
#                              if cluster_dict[edge[0]] != cluster_dict[edge[1]]]
    
#     # Find the maximum number of inter-community edges that can be removed
#     max_removable = 0
#     for i in range(1, len(inter_community_edges) + 1):
#         temp_graph = graph.copy()
#         temp_graph.remove_edges_from(inter_community_edges[:i])
#         if nx.is_connected(temp_graph):
#             max_removable = i
#         else:
#             break
    
#     # Adjust comm_delete if necessary
#     actual_comm_delete = min(comm_delete, max_removable / len(inter_community_edges))
    
#     # Delete inter-community edges
#     num_delete = int(len(inter_community_edges) * actual_comm_delete)
#     edges_to_delete = random.sample(inter_community_edges, num_delete)
#     graph.remove_edges_from(edges_to_delete)
    
#     # Add intra-community edges
#     num_add = int(initial_edges * comm_add)
#     edges_added = 0
#     while edges_added < num_add:
#         community = random.choice(communities)
#         if len(community) < 2:
#             continue
#         node1, node2 = random.sample(list(community), 2)
#         if not graph.has_edge(node1, node2):
#             graph.add_edge(node1, node2)
#             edges_added += 1
    
#     # Update edge_index in the original data
#     new_edges = list(graph.edges())
#     new_edge_index = torch.tensor(new_edges + [(v, u) for u, v in new_edges]).t().contiguous()
...
#     end_time = time.time()
#     rewiring_time = end_time - start_time
    
#     print(f"Initial number of edges: {initial_edges}")
#     print(f"Edges deleted: {num_delete}")
#     print(f"Edges added: {edges_added}")
#     print(f"Final number of edges in NetworkX graph: {graph.number_of_edges()}")
#     print(f"Final number of directed edges in PyG format: {new_edge_index.shape[1]}")
#     print(f"FinalGap = {fgap}")
#     print(f"Rewiring time: {rewiring_time:.2f} seconds")
    
#     return data, fgap, num_delete,edges_added,rewiring_time
...
def community_rewiring(data, seed, comm_delete, comm_add):
    ...
    # Use the specified number of edges to delete, but don't exceed max_removable
    num_delete = min(comm_delete, max_removable)
    ...
    edges_added = 0
    attempts = 0
    max_attempts = num_add * 10  # Limit the number of attempts to avoid infinite loops

    while edges_added < num_add and attempts < max_attempts:
        community = random.choice(communities)
        if len(community) < 2:
            attempts += 1
            continue
...
def inverse_community_rewiring(data, seed, intra_delete, inter_add):
    start_time = time.time()
    
    random.seed(seed)
    np.random.seed(seed)
    print("Inverse rewiring using community detection...")
    ...
    intra_community_edges = [edge for edge in graph.edges() 
                             if cluster_dict[edge[0]] == cluster_dict[edge[1]]]
    
    # Find the maximum number of intra-community edges that can be removed
    max_removable = 0
    for i in range(1, len(intra_community_edges) + 1):
        temp_graph = graph.copy()
        temp_graph.remove_edges_from(intra_community_edges[:i])
        if nx.is_connected(temp_graph):
            max_removable = i
        else:
            break
    ...
    # Use the specified number of edges to delete, but don't exceed max_removable
    num_delete = min(intra_delete, max_removable)
    
    # Delete intra-community edges
    edges_to_delete = random.sample(intra_community_edges, num_delete)
    graph.remove_edges_from(edges_to_delete)
    ...
    while edges_added < num_add and attempts < max_attempts:
        ...
        if community1 == community2:
            attempts += 1
            continue
        node1 = random.choice(list(community1))
        node2 = random.choice(list(community2))
        if not graph.has_edge(node1, node2):
            graph.add_edge(node1, node2)
            edges_added += 1
        attempts += 1
    ...
    print(f"Initial number of edges: {initial_edges}")
    print(f"Intra-community edges deleted: {num_delete}")
    print(f"Inter-community edges added: {edges_added}")
    print(f"Final number of edges in NetworkX graph: {graph.number_of_edges()}")
    print(f"Final number of directed edges in PyG format: {new_edge_index.shape[1]}")
    print(f"FinalGap = {fgap}")
    print(f"Rewiring time: {rewiring_time:.2f} seconds")
