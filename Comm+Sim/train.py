...
def train_and_get_results(data, model, p, lr, seed, splits, weight_decay=5e-4):
    ...
    return np.mean(test_acc_allsplits), 2 * np.std(test_acc_allsplits) / np.sqrt(len(test_acc_allsplits)),np.mean(final_val_acc),2 * np.std(val_acc_allsplits) / np.sqrt(len(val_acc_allsplits))
